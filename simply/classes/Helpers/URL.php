...
class URL {
	 ...
	 * @return  string
	 * @uses    \Simply::$index_file
	 * @uses    \Core\Request::protocol()
	 */
	public static function base($protocol = NULL, $index = FALSE)
	{
		// Start with the configured base URL
		$base_url = \Simply::$base_url;

		if ($protocol === TRUE)
		{
			// Use the initial request to get the protocol
			$protocol = \Core\Request::$initial;
		}

		if ($protocol instanceof \Core\Request)
		{
			if ( ! $protocol->secure())
			{
		...
		if ($index === TRUE AND ! empty(\Simply::$index_file))
		{
			// Add the index file to the URL
			$base_url .= \Simply::$index_file.'/';
		}
	 ...
	 * Used by URL::site()
	 *
	 * @param  array $matches  \Helpers\Array of matches from preg_replace_callback()
	 ...
	 *
	 * @param   array    $params   \Helpers\Array of GET parameters
	...
	public static function query(array $params = NULL, $use_get = TRUE)
	{
		if ($use_get)
		{
			...
			else
			{
				...
				$params = \Helpers\Arr::merge($_GET, $params);
			}
		}
	 ...
	 * @return  string
	 * @uses    \Helpers\UTF8::transliterate_to_ascii
	 */
	public static function title($title, $separator = '-', $ascii_only = FALSE)
	{
		if ($ascii_only === TRUE)
		{
			...
			$title = \Helpers\UTF8::transliterate_to_ascii($title);
		...
		else
		{
			...
			$title = preg_replace('![^'.preg_quote($separator).'\pL\pN\s]+!u', '', \Helpers\UTF8::strtolower($title));
		}
