 ...
 *
 * [!!] This file is licensed differently from the rest of Simply Framework. As a port of
...
class UTF8 {
	 ...
	 */
	protected static $server_utf8 = NULL;
	...
	public static function server_utf8()
	{
		if (static::$server_utf8 === NULL)
		{
			// Determine if this server supports UTF-8 natively
			static::$server_utf8 = extension_loaded('mbstring');
		}

		return static::$server_utf8;
	}
	...
	public static function clean($var)
	{
		...
		elseif (is_string($var) AND $var !== '')
		{
			...
			if ( ! static::is_ascii($var))
			{
				...
				$var = iconv(static::CHARSET, static::CHARSET.'//IGNORE', $var);
	...
	public static function strcspn($str, $mask, $offset = NULL, $length = NULL)
	{
		...
		if (UTF8::is_ascii($str) AND UTF8::is_ascii($mask))
		{
			...
			elseif ($length === NULL)
			{
				return strcspn($str, $mask, $offset);
			}
			else
			{
	...
	public static function str_pad($str, $final_str_length, $pad_str = ' ', $pad_type = STR_PAD_RIGHT)
	{
		...
		throw new \Helpers\UTF8\Exception("UTF8::str_pad: Unknown padding type (:pad_type)", array(
				':pad_type' => $pad_type,
			));
	}
	...
	public static function ord($chr)
	{
		...
		if ( ! isset($chr[1]))
		{
			throw new \Helpers\UTF8\Exception('Short sequence - at least 2 bytes expected, only 1 seen');
		}
		...
		if ( ! isset($chr[2]))
		{
			throw new \Helpers\UTF8\Exception('Short sequence - at least 3 bytes expected, only 2 seen');
		}
		...
		if ( ! isset($chr[3]))
		{
			throw new \Helpers\UTF8\Exception('Short sequence - at least 4 bytes expected, only 3 seen');
		}
		...
		if ( ! isset($chr[4]))
		{
			throw new \Helpers\UTF8\Exception('Short sequence - at least 5 bytes expected, only 4 seen');
		}
		...
		if ( ! isset($chr[5]))
		{
			throw new \Helpers\UTF8\Exception('Short sequence - at least 6 bytes expected, only 5 seen');
		}
		...
		{
			throw new \Helpers\UTF8\Exception("Invalid UTF-8 with surrogate ordinal ':ordinal'", array(
				':ordinal' => $ord0,
			));
		}
	}
		...
		for ($i = 0; $i < $len; $i++)
		{
			...
			else
			{
				...
				else
				{
					...
					throw new \Helpers\UTF8\Exception("UTF8::to_unicode: Incomplete multi-octet sequence in UTF-8 at byte ':byte'", array(
						':byte' => $i,
					));
				}
			}
	...
	public static function from_unicode($arr)
	{
		...
		foreach ($keys as $k)
		{
			...
			{
				// Found a surrogate
				throw new \Helpers\UTF8\Exception("UTF8::from_unicode: Illegal surrogate at index: ':index', value: ':value'", array(
					':index' => $k,
					':value' => $arr[$k],
				));
			}
			...
			{
				throw new \Helpers\UTF8\Exception("UTF8::from_unicode: Codepoint out of Unicode range at index: ':index', value: ':value'", array(
					':index' => $k,
					':value' => $arr[$k],
				));
			}
