...
class Feed {
	 ...
	 *
	 * @param   string  $feed   remote feed \Helpers\URL
	...
	public static function parse($feed, $limit = 0)
	{
		...
		{
			throw new \Core\Exception('SimpleXML must be installed!');
		}
		...
		if (\Helpers\Valid::url($feed))
		{
			// Use native Request client to get remote contents
			$response = \Core\Request::factory($feed)->execute();
			$feed     = $response->body();
		}
		elseif (is_file($feed))
		{
	...
	public static function create($info, $items, $encoding = 'UTF-8')
	{
		...
		foreach ($info as $name => $value)
		{
			if ($name === 'image')
			{
				...
				{
					throw new \Core\Exception('Feed images require a link, url, and title');
				}

				if (strpos($value['link'], '://') === FALSE)
				{
					// Convert URIs to \Helpers\URLs
					$value['link'] = \Helpers\URL::site($value['link'], 'http');
				}

				if (strpos($value['url'], '://') === FALSE)
				{
					// Convert URIs to \Helpers\URLs
					$value['url'] = \Helpers\URL::site($value['url'], 'http');
				}
			...
			{
				if (
					($name === 'pubDate' OR $name === 'lastBuildDate')
					AND (is_int($value) OR ctype_digit($value))
				)
				{
				...
				{
					// Convert URIs to \Helpers\URLs
					$value = \Helpers\URL::site($value, 'http');
				}
