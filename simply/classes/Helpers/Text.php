...
/**
 * Text helper class.
 * Provides simple methods for working with text.
 */
class Text {
	 ...
	 *
	 *     $text = \Helpers\Text::limit_words($text);
	 *
	 ...
	 *
	 *     $text = \Helpers\Text::limit_chars($text);
	 *
	 ...
	 *
	 *     echo \Helpers\Text::alternate('one', 'two'); // "one"
	 *     echo \Helpers\Text::alternate('one', 'two'); // "two"
	 *     echo \Helpers\Text::alternate('one', 'two'); // "one"
	 *
	 ...
	 *
	 *
	 *     $str = \Helpers\Text::random(); // 8 character random string
	 *
	 ...
	 *
	 *      $str = \Helpers\Text::ucfirst('content-type'); // returns "Content-Type"
	 *
	 ...
	 *
	 *     $str = \Helpers\Text::reduce_slashes('foo//bar/baz'); // "foo/bar/baz"
	 *
	 ...
	 *     echo \Helpers\Text::censor('What the frick, man!', array(
	 *         'frick' => '#####',
	 *     ));
	 *
	 ...
	 *
	 *     $match = \Helpers\Text::similar(array('fred', 'fran', 'free'); // "fr"
	 *
	 ...
	 * will not be altered.
	 *
	 *     echo \Helpers\Text::auto_link($text);
	 *
	 * [!!] This method is not foolproof since it uses regex to parse HTML.
	 *
	 * @param   string  $text   text to auto link
	 * @return  string
	 * @uses    \Helpers\Text::auto_link_urls
	 * @uses    \Helpers\Text::auto_link_emails
	 */
	public static function auto_link($text)
	{
		// Auto link emails first to prevent problems with "www.domain.com@example.com"
		return static::auto_link_urls(\Helpers\Text::auto_link_emails($text));
	}

	/**
	 * Converts text anchors into links. Existing links will not be altered.
	 *
	 *     echo \Helpers\Text::auto_link_urls($text);
	 *
	 * [!!] This method is not foolproof since it uses regex to parse HTML.
	 *
	 * @param   string  $text   text to auto link
	...
	public static function auto_link_urls($text)
	{
		...
		$text = preg_replace_callback('~\b(?<!href="|">)(?:ht|f)tps?://[^<\s]+(?:/|\b)~i', '\Helpers\Text::_auto_link_urls_callback1', $text);

		// Find and replace all naked www.links.com (without http://)
		return preg_replace_callback('~\b(?<!://|">)www(?:\.[a-z0-9][-a-z0-9]*+)+\.[a-z]{2,6}[^<\s]*\b~i', '\Helpers\Text::_auto_link_urls_callback2', $text);
	}
	 ...
	 * be altered.
	 *
	 *     echo \Helpers\Text::auto_link_emails($text);
	 *
	 * [!!] This method is not foolproof since it uses regex to parse HTML.
	 *
	 * @param   string  $text   text to auto link
	...
	public static function auto_link_emails($text)
	{
		...
		return preg_replace_callback('~\b(?<!href="mailto:|58;)(?!\.)[-+_a-z0-9.]++(?<!\.)@(?![-.])[-a-z0-9.]+(?<!\.)\.[a-z]{2,6}\b(?!</a>)~i', '\Helpers\Text::_auto_link_emails_callback', $text);
	}
	 ...
	 *
	 *     echo \Helpers\Text::bytes(filesize($file));
	 *
	 ...
	 *     echo \Helpers\Text::number(1024);
	 *
	 *     // Display: five million, six hundred and thirty-two
	 *     echo \Helpers\Text::number(5000632);
	 *
	...
	public static function user_agent($agent, $value)
	{
		...
		if ($value === 'browser' OR $value == 'version')
		{
			...
			// Load browsers
			$browsers = \Simply::$config->load('user_agents')->browser;
			...
			foreach ($browsers as $search => $name)
			{
				if (stripos($agent, $search) !== FALSE)
				{
					...
					if (preg_match('#'.preg_quote($search).'[^0-9.]*+([0-9.][0-9.a-z]*)#i', \Core\Request::$user_agent, $matches))
					{
		...
		else
		{
			...
			$group = \Simply::$config->load('user_agents')->$value;
