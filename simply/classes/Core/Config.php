...
class Config {
	 ...
	 *
	 * @param   \Core\Config\Source    $source instance
	 * @param   boolean                 $first  add the reader as the first used object
	 * @return  $this
	 */
	public function attach(\Core\Config\Source $source, $first = TRUE)
	{
		if ($first === TRUE)
		{
	 ...
	 * @return  $this
	 */
	public function detach(\Core\Config\Source $source)
	{
	 ...
	 *
	 * See [\Core\Config\Group] for more info
	 *
	 * @param   string  $group  configuration group name
	 * @return  \Core\Config\Group
	...
	public function load($group)
	{
		...
		{
			throw new \Core\Exception('No configuration sources attached');
		}

		if (empty($group))
		{
			throw new \Core\Exception("Need to specify a config group");
		}

		if ( ! is_string($group))
		{
			throw new \Core\Exception("Config group must be a string");
		}
		...
		if (isset($this->_groups[$group]))
		{
			if (isset($path))
			{
				return \Helpers\Arr::path($this->_groups[$group], $path, NULL, '.');
			}
		...
		{
			if ($source instanceof \Core\Config\Reader)
			{
				if ($source_config = $source->load($group))
				{
					$config = \Helpers\Arr::merge($config, $source_config);
				}
			}
		}

		$this->_groups[$group] = new \Core\Config\Group($this, $group, $config);

		if (isset($path))
		{
			return \Helpers\Arr::path($config, $path, NULL, '.');
		}
	...
	public function _write_config($group, $key, $value)
	{
		...
		{
			if ( ! ($source instanceof \Core\Config\Writer))
			{
				continue;
			}
